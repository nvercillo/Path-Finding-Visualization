...
def about():
    ...
    while line:
        ...
        airports[line[inds[3] + 2 : inds[4] - 1]]["country"] = line[
            inds[2] + 2 : inds[3] - 1
        ]
        airports[line[inds[3] + 2 : inds[4] - 1]]["coords"] = {}
        airports[line[inds[3] + 2 : inds[4] - 1]]["coords"]["latitude"] = float(
            line[inds[5] + 1 : inds[6]]
        )
        airports[line[inds[3] + 2 : inds[4] - 1]]["coords"]["longitude"] = float(
            line[inds[6] + 1 : inds[7]]
        )
        airports[line[inds[3] + 2 : inds[4] - 1]]["name"] = line[
            inds[0] + 2 : inds[1] - 1
        ]
        line = f.readline()
    ...
    while line:
        ...
        for i in range(0, len(line)):
            if line[i] == ",":
                inds.append(i)
        if (
            line[inds[1] + 1 : inds[2]] not in set
            and line[inds[3] + 1 : inds[4]] not in set
        ):
